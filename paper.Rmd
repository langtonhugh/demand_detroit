...
For each incident, the police response time and time on the scene are reported. The former measures the response time from dispatch to when the first officer arrives on-scene.^[In the raw data, response time is referred to as _traveltime_. This figure does not include the intake or dispatch time which we did not deem to be deployment.] The latter mimics the measure used by @lum2021can, namely, the amount of time which elapses between when the first officer arrives and when the last unit clears the scene. Here, we refer to the sum of the response time and time on scene as the _total deployed time_. Calls which had a response time of negative values or missing were dropped from analyses. Calls which had a _time on scene_ of zero minutes, negative values or missing were also excluded. The data permit a distinction between the time spent with and without the response time. As noted, @lum2021can considered the _time on scene_ measure to be preferable. Here, for completeness, we report times with and without the response time.
...
On this point, this study raises useful discussion points around the different measures of officer time. First, it is clear that findings on the scale and composition of police demand are sensitive, albeit only slightly, to whether measures of officer time include the response time. The degree to which the two estimates differ varies by demand classification, likely as a result of the different call priorities, and in turn, the rapidness of the response times. That said, this claim would need verifying through detailed examination. On this point, second, we would reiterate that the 'time on scene' measure reported here, in alignment with @lum2021can, refers to the amount of time which elapses between when the first officer arrives and when the last unit clears the scene. This differs to that of @ratcliffe2021policing which accounted the number of officers attending the scene. @lum2021can note that including back-up officers in the time calculation might introduces complications. For instance, non-serious incidents might receive back-up unnecessarily, particularly during periods of low-demand, which would inflate the time consumed and overstate the resources required to resolve such an incident. Although the overall composition of police demand is comparable between Philadelphia [@ratcliffe2021policing] and Detroit, the true impact of these measures remains unclear. We propose that future research investigates these distinctions. This would require CAD (or equivalent) data, timestamped at each stage the response (call-handler dispatch, arrival on scene, scene departure), for individual police units nested within calls. This level of granularity would permit the calculation and comparison of multiple measures of officer time, as well as an investigation into the extent to which back-ups and/or 'flocking' might influence such measures.
