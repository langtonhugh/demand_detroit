---
title: 'Describing the scale and composition of calls for police service: a replication and
...
header-includes:
  ...
  - \linenumbers
csl: apa.csl
abstract: "This paper describes the scale and composition of emergency public demand for police services in Detroit, United States. The contribution is made in replication and extension of analyses reported by Ratcliffe (2021) and Lum et al. (2021). Findings indicate that police spend a considerable proportion of time performing a social service function. Just 47% of total deployed time responding to 911 calls is consumed by crime incidents. The remainder is spent on community issues (6%), health (7%), proactive (5%), quality of life (19%) and traffic duties (13%). A small number of calls consume a disproportionately large amount of police officer time. Emergency demand is concentrated in time and space, and can differ between types of demand. Findings further highlight the potential implications of radically reforming police forces in the United States. In advance of existing studies, the data and code used here are openly available for reproduction, re-use and scrutiny."
---
...
\dag Netherlands Institute for the Study of Crime and Law Enforcement, Amsterdam, Netherlands. \linebreak \linebreak \linebreak \linebreak
...
**Funding details** To be completed.
...
Here, we propose an extension of @ratcliffe2021policing and @lum2021can. This extension is motivated by three principal components. First, we replicate the main analyses of these existing investigations into the scale and composition of police demand using open study materials. To date, investigations have been conducted on data which is not publicly available and without the materials (e.g., code) to reproduce the results. Yet, open science practices have been identified as fundamental to the aims of evidence-based policing and vital in ensuring that real-life decisions are made based on a concrete pool of evidence [@openedp2019]. In this case. the necessity for a fully open investigation into the scale and composition of police demand is perhaps especially important given the public nature of debate around police reform and the considerable implications of redefining the role of the police without transparent empirical investigation. It is reasonable for police practitioners and the public to expect a thorough, open and replicable investigation into police demand across different contexts before academia concludes that the issue is resolved – certainly before findings are used to change or introduce police practice or public policy.

Second, we extend the analyses used by @ratcliffe2021policing and @lum2021can. This includes a direct comparison between different measures of 'officer time', namely, total deployment time and time on scene. These measures have been reported separately in each existing analyses, so the precise impact of their usage on the same data is unclear. We also provide a comprehensive account of the variability in the time consumed by different demand classifications (e.g., crime, community, health). These statistics shed light on the degree to which a disproportionately large amount of officer time is consumed by just a small number of calls - insight which can be used to assess how a reduction in demand, or a shift away from calls for police service towards other social services, might free-up officer time. In addition, we report on the spatial and temporal patterning of each demand classification. Distinctions in these patterns across demand types would suggest similarly distinct demand-generating mechanisms (e.g,, environmental characteristics) and inform deployment decisions based on officer expertise and experience, and/or collaboration with other services (e.g., mental health professionals).



That said, a notable shortcoming of these recent contributions is the lack of openly available data and code to reproduce and replicate findings. This is not an uncommon issue in policing research. A recent scoping review of Canadian policing studies concluded that there has been "little effort... to replicate or reproduce findings", with just 8% of the literature sampled constituting replications or reproductions [@huey2017replication, p. 113]. More broadly, in a sample of papers from top criminology journals, @mcneeley2015replication found that just 2% of papers were replications -- either a direct replication of the same population, or empirical generalizations from a different population. Yet, the practice of open science is fundamental in achieving the goals of evidence-based policing, namely, "to develop a rigorous evidence base that can be used by police professionals to develop more efficient and effective police practices, policies, and/or programs" [@openedp2019, p. 126]. Comparable statements have been made in related fields, such as science [@laycock2013defining]. It therefore seems appropriate that the investigations of @ratcliffe2021policing and @lum2021can are replicated using open data and methods.

One reason to hesitate over conducting such a replication is that these two existing studies were descriptive accounts of police demand in specific study sites. In scientific research more broadly, replications are often motivated by the need to check for Type I or Type II errors using a different sample from the same population and/or adjust for control variables which may have influenced the dependent variable of interest [@gross1997need; see also @mackey2012or]. We might therefore conclude that a replication of @ratcliffe2021policing and @lum2021can is unwarranted: instead, replications should be reserved only for those studies deploying experimental designs or inferential statistics from a random sample, the conclusions from which will be generalized to a population. We disagree. There are plentiful reasons to conduct an open descriptive investigation into the scale and composition of police demand. This is for two principal reasons.

the descriptive findings from @lum2021can, without the use of random sampling and inferential statistics, are already being generalized to the United States as a whole. This is most evident in _The Washington Post_ article [@lumwash].


the implications of redefining the role of the police is considerable.


It is reasonable for police practitioners and the public to expect a thorough, open and replicable empirical investigation into police demand across numerous contexts before academia concludes that the issue is resolved. B

certainly before findings are used to change or introduce police practice or public policy. As noted, committing to radical reform without such investigations might result in shifts in public demand for services for which there is no suitable supply.


Second, the data handling required to describe the public demand for the police is not straightforward. Reporting the scale and composition of emergency calls for service data necessitates the grouping of incident types through categorization (e.g., 'mental health', 'community'). As both @ratcliffe2021policing and @lum2021can point out, categorizing incident types is challenging and subject to a degree of interpretation. This issue is exacerbated by the fact that different police jurisdictions will use different incident categories. Thus, an open replication, inclusive of the categorization decisions, facilitates scrutiny through transparency and provides robustness to existing findings.

With these points in mind, this paper describes the emergency demand for police services in a large city, Detroit, US, using open data and open code. All materials are publicly available (see https://anonymous.4open.science/r/demand_detroit-FCD3/README.md). It does so in replication, and where appropriate extension, of the main findings reported by @ratcliffe2021policing and later @lum2021can. In this way, the contribution serves to test the robustness of existing findings, but also provides the materials required to reproduce and replicate analyses in the interests of transparency and public scrutiny.
...
Recognising that frequency counts of calls for service do not necessarily reflect the amount of police resource consumed by public demand, raw counts were augmented with the time spent by officers on each of the six classifications of demand. This information permits the calculation of a proportional breakdown of police time consumed by the different demand classifications throughout the year. The raw numbers were reported in a table. A visual representation of call types nested within each demand classification was summarised in a tree map graphic. The time committed by officers was calculated from when the call was dispatched to an officer to when the officer indicated that the event has been closed. Here, this is referred to as the _total deployed time_. The aggregate time consumed by different demand classifications was reported using a single count of officer shifts (7.5 hours).

The spatio-temporal patterning of this demand was summarised in two visuals, focusing specifically on those calls relating to medical/public health. A heatmap showed the mean call volume during hours of the day and days of the week, identifying peak times for demand. A geographic map visualized the spatial patterning of these call count concentrations using kernel density in order to identify hotspots of police demand involving medical/public health issues. To supplement these findings, Ratcliffe also reported on the differences between the initial CAD incident type classification and the final disposition using a sankey diagram -- again focusing specifically on medical and public health calls.
...
kable(table1_df, caption = "Summary of the various descriptions of demand replicated or extended in this study. Measures were reported for demand classification unless otherwise stated.",
  ...
  column_spec(column = c(2,3), width = c("2cm", "2cm") ) %>% 
